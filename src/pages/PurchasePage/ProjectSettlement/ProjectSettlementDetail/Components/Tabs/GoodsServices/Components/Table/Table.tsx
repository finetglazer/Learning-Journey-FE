...
import { Goods, GoodsCategoryCosts } from "models/ProjectSettlement";
...
const flattenGroupedData = (data: ModelData[]): Goods[] => {
  return data.flatMap((categoryData) => categoryData.items);
};
...
export const Table = () => {
  const { model, dispatch } = useProjectSettlementDetailContext();
  ...
  const calculateTotalCostValues = (data: GoodsCategoryCosts[]) => {
    return data.reduce(
      (totals, item) => {
        return {
          totalInvestmentBeforeTax:
            totals.totalInvestmentBeforeTax + item.investmentBeforeTax,
          totalInvestmentTax: totals.totalInvestmentTax + item.investmentTax,
          totalInvestmentSum: totals.totalInvestmentSum + item.investmentSum,
          totalSettlementBeforeTax:
            totals.totalSettlementBeforeTax + item.settlementBeforeTax,
          totalSettlementTax: totals.totalSettlementTax + item.settlementTax,
          totalSettlementSum: totals.totalSettlementSum + item.settlementSum,
          totalDifferenceValue:
            totals.totalDifferenceValue + item.differenceValue,
        };
      },
  ...
  const handleOnSave = (item: Goods) => {
    // dismiss drawer before saving
    onDismissDrawer();
    // update new item to data model
    // because the data is array key - array value, we need find item in array value
    const dataCopy = [...data];

    // we have special case, the good duplicate id so we need add conditional is unitId
    for (const categoryData of dataCopy) {
      const index = categoryData.items.findIndex(
        (good) =>
          isEqual(good.goodsId, item.goodsId) &&
          isEqual(good?.goodsServiceUnit?.id, item?.goodsServiceUnit?.id)
      );
      if (!isEqual(index, -numberConstants.ONE)) {
        const { settlementBeforeTax, settlementTax, note } = item;
        Object.assign(categoryData.items[index], {
          settlementBeforeTax,
          settlementTax,
          note,
        });
    ...
    const newData = flattenGroupedData(dataCopy);

    // we need update data: goodsCategoryCosts and investmentCosts
    const updatedGoodsCategoryCosts = model.goodsCategoryCosts.map(
      (category) => {
        const goodsData = newData.find((item) =>
          isEqual(item?.goodsServicesCategory?.id, category?.goodsCategoryId)
        );
        ...
        if (goodsData) {
          return {
            ...category,
            note: goodsData?.note,
            settlementBeforeTax: goodsData?.settlementBeforeTax,
            settlementTax: goodsData?.settlementTax,
            settlementSum: goodsData?.settlementSum,
            differenceValue: goodsData?.differenceValue,
          };
        }
    ...
    const totalCosts = calculateTotalCostValues(updatedGoodsCategoryCosts);

    const investmentCosts = model.investmentCosts.map(
      (investmentCost, index) => {
        if (isEqual(index, numberConstants.ZERO)) {
          return {
            ...investmentCost,
            differenceValue: totalCosts.totalDifferenceValue,
            investmentBeforeTax: totalCosts.totalInvestmentBeforeTax,
            investmentTax: totalCosts.totalInvestmentTax,
            investmentSum: totalCosts.totalInvestmentSum,
            settlementBeforeTax: totalCosts.totalSettlementBeforeTax,
            settlementTax: totalCosts.totalSettlementTax,
            settlementSum: totalCosts.totalSettlementSum,
          };
        }
    ...
    const newModel = {
      ...model,
      projectSettlementGoodsItems: newData,
      goodsCategoryCosts: updatedGoodsCategoryCosts,
      investmentCosts,
    };
