...
const INFORMATION_KEY = "information_settlement_goods_services";

const DEFAULT_VALUE = "---";

const InformationForm = () => {
  const [translate] = useTranslation();
  const { model } = useContext<DrawerInformationHookTye>(
    DrawerInformationContext
  );
  ...
  const makeSettlementRow = () => {
    return (
      <tr className={styles["special-row"]}>
        ...
        <th>
          {formatCurrency(model?.settlementBeforeTax || numberConstants.ZERO)}
        </th>
        <th>
          {formatCurrency(
            model?.settlementSum - model?.settlementBeforeTax ||
    ...
    <>
      ...
      <div className={styles["note-wrapper"]}>
        <div className={styles["label"]}>
          {translate("PS.txt_note_settlement_goods_services")}
        </div>

        <div className={styles["value"]}>{model?.note || DEFAULT_VALUE}</div>
      </div>
    </>
  );
};
    ...
    <DrawerInformationContext.Provider value={contextValue}>
      ...
      >
        <div className={styles[""]}>
          <AdvancedCollapseView
            isFullView
            defaultActiveKey={INFORMATION_KEY}
            items={[
              {
                key: INFORMATION_KEY,
                label: translate(
                  "PS.txt_information_settlement_goods_services"
                ),
                children: makeInformation(),
              },
            ]}
          />
          {makeForm()}
