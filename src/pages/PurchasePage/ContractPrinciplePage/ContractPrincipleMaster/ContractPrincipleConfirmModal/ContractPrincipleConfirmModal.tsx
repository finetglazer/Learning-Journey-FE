import { CancelRoundIcon, TrashRoundIcon } from "assets/icons";
import { isNil } from "lodash";
import { ContractPrinciple } from "models/ContractPrinciple";
import { useCallback, useMemo, useState } from "react";
import {
  FormItem,
  ModalConfirm,
  TextArea,
} from "react-components-design-system";
import { Trans, useTranslation } from "react-i18next";

const MAX_LENGTH_REASON = 500;
const ZERO = 0;

export enum ConfirmModalType {
  DELETE = "DELETE",
  CANCEL = "CANCEL",
}

interface ModalProps {
  title: string;
  content: string[];
  icon: string;
  reasonLabel: string;
}

interface ContractPrincipleConfirmModalProps {
  type: ConfirmModalType;
  model?: ContractPrinciple;
  isLoading: boolean;
  errorMessage?: string;
  onApply?: (proposal: ContractPrinciple, reason: string) => void;
  onCancel?: () => void;
}

export const ContractPrincipleConfirmModal = ({
  type,
  model,
  isLoading,
  errorMessage,
  onApply,
  onCancel,
}: ContractPrincipleConfirmModalProps) => {
  const [translate] = useTranslation();

  const [reason, setReason] = useState<string>("");

  const mapModelToProps = useCallback(
    (type: ConfirmModalType): ModalProps => {
      const name = translate("CT.txt_contract_principles_lower_case");

      const modalProps = {
        [ConfirmModalType.DELETE]: {
          title: translate("CT.modal_contract_principles.title.delete", {
            name,
          }),
          content: ["modal_contract_principles.content.delete", model.code],
          icon: TrashRoundIcon,
          reasonLabel: translate("BG.txt_reason_delete"),
        },
        [ConfirmModalType.CANCEL]: {
          title: translate("CT.modal_contract_principles.title.cancel", {
            name,
          }),
          content: ["modal_contract_principles.content.cancel", model.code],
          icon: CancelRoundIcon,
          reasonLabel: translate("BG.txt_reason_cancel"),
        },
      };

      return modalProps[type];
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [translate, model.code]
  );

  const modalProps = useMemo(
    () => mapModelToProps(type),
    [mapModelToProps, type]
  );

  const onChangeText = (text: string) => {
    const trimmedValue = text.trim();

    setReason(trimmedValue);
  };

  const onSave = () => {
    if (isNil(onApply)) return;

    onApply(model, reason);
  };

  return (
    <ModalConfirm
      open
      title={modalProps?.title}
      content={
        <Trans
          i18nKey={`CT.${modalProps?.content[ZERO]}`}
          values={{ code: model.code }}
        />
      }
      loading={isLoading}
      icon={<img src={modalProps?.icon} alt="" />}
      titleButtonApply={translate("CM.btn_confirm")}
      titleButtonCancel={translate("CM.btn_close")}
      handleSave={onSave}
      handleCancel={onCancel}
    >
      {/* TextArea */}
      <div className="w-100 h-100 m-t--lg">
        <FormItem
          message={errorMessage || ""}
          validateStatus={isNil(errorMessage) ? undefined : "error"}
        >
          <TextArea
            isRequired
            showCount
            maxLength={MAX_LENGTH_REASON}
            label={modalProps.reasonLabel}
            placeHolder={translate("BG.input_reason")}
            value={reason}
            onChange={onChangeText}
            translate={translate}
          />
        </FormItem>
      </div>
    </ModalConfirm>
  );
};
