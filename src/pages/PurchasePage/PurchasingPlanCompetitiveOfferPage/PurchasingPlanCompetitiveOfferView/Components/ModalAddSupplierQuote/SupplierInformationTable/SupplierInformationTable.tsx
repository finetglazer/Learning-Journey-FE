...
const SupplierInformationTableBidding = ({
  ...
  const groupBySupplierIndexNumber = (obj: any) => {
    const result: any = {};

    Object.keys(obj).forEach((key) => {
      const match = key.match(/^suppliers\[(\d+)\]\.(.+)$/);

      if (match) {
        const index = parseInt(match[1]); // Convert to number
        ...
        result[index][fieldName] = obj[key];
      }
    });
  ...
  const groupedErrors = useMemo(() => {
    console.log("groupedErrors", errors);
    return groupBySupplierIndexNumber(errors);
  }, [errors]);

  const [dataSource, setDataSource] = useState([]);

  useEffect(() => {
    const data = getDataSource(actionQuote)?.map(
      (item: SupplierModel, index: number) => {
        const errors = groupedErrors[index] || {};
        return {
          ...item,
          errors: errors,
          key: item.id,
  ...
  const getMapFieldErrors: any = {
    quoteEmail: translate("PL.drawer_email_person_quoting_price"),
  };

  const renderIconWarning = useCallback(
    (_: any, record: any) => {
      if (isEmpty(record?.errors)) return null;
      // Tạo nội dung tooltip từ các errors
      const errorMessages = Object.entries(record?.errors)
        .map(
          ([field, message]) =>
            `${getMapFieldErrors[field] ?? field}: ${message}`
        )
        .join("\n");
      ...
      return [
        {
          ...
          type: "link",
          width: 40,
          hidden: isEmpty(groupedErrors) || size(groupedErrors) === 0,
          render: renderIconWarning,
        },
        {
          title: (
            <UnitTitle
