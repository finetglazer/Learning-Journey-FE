...
const RenderDetail = ({
  ...
  useEffect(() => {
    const valueEmailObjectDefault =
      supplierContacts?.filter((item) => item?.isDefault) || [];
    if (
      isEmpty(model?.id) &&
      size(valueEmailObjectDefault) > 0 &&
      isEmpty(valueEmailObject?.id)
    ) {
      handleChangeEmail(valueEmailObjectDefault[0]);
    }
  }, [supplierContacts, visible]);
