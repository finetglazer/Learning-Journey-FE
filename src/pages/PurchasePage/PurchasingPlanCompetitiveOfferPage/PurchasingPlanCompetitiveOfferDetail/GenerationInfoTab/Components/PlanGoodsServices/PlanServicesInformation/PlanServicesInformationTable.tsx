...
import PlanGoodServicesDrawer from "pages/PurchasePage/PurchasingPlanCompetitiveOfferPage/PurchasingPlanCompetitiveOfferDetail/Components/PlanGoodServicesDrawer/PlanGoodServicesDrawer";
import {
  convertData,
  convertDataWithErrors,
} from "pages/PurchasePage/PurchasingPlanPage/PurchasingPlanDetail/PurchasePlanGenerationInfoTab/Components/Helper";
...
const PlanServicesInformationTable = ({
  ...
  const clearGoodsItemsErrors = (errors: any, indexList: number[]) => {
    if (isNil(errors)) return {};

    const errorsClone = cloneDeep(errors);

    Object.keys(errorsClone).forEach((key) => {
      if (indexList.some((index) => key.startsWith(`goodsItems[${index}]`))) {
  ...
  const handleDeleteRow = () => {
    ...
    const item = model?.purchaseItems?.find((item) => item.id === idDelete);
    const ids = selectedRowKeys.filter((id) => id !== idDelete);
    const newErrors = clearGoodsItemsErrors(model?.errors, [
      item?.indexBeforeValidate,
    ]);
    setSelectedRowKeys(ids);
    handleChangeAllField({
      ...model,
      purchaseItems: editSelectedGoods,
      errors: newErrors,
    });
    handleChangeSingleField({
  ...
  const handleBulkDeleteRow = () => {
    ...
    const listIndex = model?.purchaseItems?.reduce((acc, item) => {
      acc.push(item?.indexBeforeValidate);
      return acc;
    }, []);

    const newErrors = clearGoodsItemsErrors(model?.errors, listIndex);
    handleChangeAllField({
      ...model,
      purchaseItems: editSelectedGoods,
      errors: newErrors,
  ...
  const getGoodsItems = (purchaseItems: PurchasePlanGoodsServicesModel[]) => {
    ...
    return Object.values(groupedByCategoryId).flat();
  };

  const convertDataByCategory = convertDataWithErrors(
    getGoodsItems(model?.purchaseItems) ?? [],
    model
  );
