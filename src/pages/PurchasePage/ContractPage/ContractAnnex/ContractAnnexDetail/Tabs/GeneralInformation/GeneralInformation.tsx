import { Tooltip } from "antd";
import { UploadCloudIcon } from "assets/icons";
import { AdvancedCollapseView } from "components";
import Attachments from "components/Attachments/Attachments";
import { Comments } from "components/Comment/Comment.stories";
import { numberConstants } from "core/config/consts";
import { TopicType } from "core/models/History";
import { isEmpty, isEqual, isNil } from "lodash";
import { useCallback, useMemo } from "react";
import { useTranslation } from "react-i18next";
import GoodsService from "../../../Components/GoodsService/GoodsService";
import { useContractAnnexDetailContext } from "../../context";
import { AnnexInformation } from "./Components/AnnexInformation";
import { BuyerInformation } from "./Components/BuyerInformation";
import { ContractInformation } from "./Components/ContractInformation";
import { DeliveryInformation } from "./Components/DeliveryInformation";
import { SellerInformation } from "./Components/SellerInformation";
import { Terms } from "./Components/Terms";
import styles from "./GeneralInformation.module.scss";

enum CollapseKey {
  AnnexInformation = "AnnexInformation",
  Grounds = "Grounds",
  ContractInformation = "ContractInformation",
  Buyer = "Buyer",
  Seller = "Seller",
  GoodsServicesChange = "GoodsServicesChange",
  Delivery = "ReceiverInformation",
  Attachment = "Attachment",
}

export const WarningCircleIcon = () => {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M10 0.282227C8.07163 0.282227 6.18657 0.854054 4.58319 1.9254C2.97981 2.99674 1.73013 4.51948 0.992175 6.30106C0.254221 8.08264 0.0611382 10.043 0.437344 11.9344C0.81355 13.8257 1.74215 15.563 3.10571 16.9265C4.46927 18.2901 6.20656 19.2187 8.09787 19.5949C9.98919 19.9711 11.9496 19.778 13.7312 19.0401C15.5127 18.3021 17.0355 17.0524 18.1068 15.449C19.1782 13.8457 19.75 11.9606 19.75 10.0322C19.7473 7.4472 18.7192 4.96884 16.8913 3.14095C15.0634 1.31306 12.585 0.284956 10 0.282227ZM9.25 5.53223C9.25 5.33331 9.32902 5.14255 9.46967 5.0019C9.61032 4.86124 9.80109 4.78223 10 4.78223C10.1989 4.78223 10.3897 4.86124 10.5303 5.0019C10.671 5.14255 10.75 5.33331 10.75 5.53223V10.7822C10.75 10.9811 10.671 11.1719 10.5303 11.3126C10.3897 11.4532 10.1989 11.5322 10 11.5322C9.80109 11.5322 9.61032 11.4532 9.46967 11.3126C9.32902 11.1719 9.25 10.9811 9.25 10.7822V5.53223ZM10 15.2822C9.7775 15.2822 9.55999 15.2162 9.37498 15.0926C9.18998 14.969 9.04578 14.7933 8.96064 14.5877C8.87549 14.3822 8.85321 14.156 8.89662 13.9377C8.94003 13.7195 9.04717 13.5191 9.2045 13.3617C9.36184 13.2044 9.56229 13.0973 9.78052 13.0538C9.99875 13.0104 10.225 13.0327 10.4305 13.1179C10.6361 13.203 10.8118 13.3472 10.9354 13.5322C11.059 13.7172 11.125 13.9347 11.125 14.1572C11.125 14.4556 11.0065 14.7417 10.7955 14.9527C10.5845 15.1637 10.2984 15.2822 10 15.2822Z"
        fill="#E18308"
      />
    </svg>
  );
};

export const GeneralInformation = () => {
  const [translate] = useTranslation();

  const { model, dispatch, handleChangeAllField } =
    useContractAnnexDetailContext();

  const makeCollapseTitle = useCallback(
    (type: CollapseKey, haveWarning?: boolean) => {
      let key = "";
      switch (type) {
        case CollapseKey.AnnexInformation:
          key = "CA.txt_information_annex";
          break;
        case CollapseKey.Grounds:
          key = "CA.txt_grounds";
          break;
        case CollapseKey.ContractInformation:
          key = "CA.txt_contract_information";
          break;
        case CollapseKey.Buyer:
          key = "CA.txt_buyer_information";
          break;
        case CollapseKey.Seller:
          key = "CA.txt_seller_information";
          break;
        case CollapseKey.GoodsServicesChange:
          key = "CA.txt_goods_services_change_information";
          break;
        case CollapseKey.Delivery:
          key = "CA.txt_delivery_information";
          break;
        case CollapseKey.Attachment:
          key = "CM.txt_attachment_files";
          break;
        default:
          key = null;
          break;
      }

      const typeParams = isEqual(
        numberConstants.ZERO,
        model?.contractInfo?.contractRequestType
      )
        ? translate("CA.txt_contract")
        : translate("CA.txt_order");

      return (
        <div className={styles["collapse-title"]}>
          {translate(key, { type: typeParams })}
          {haveWarning ? (
            <Tooltip title={translate("CA.txt_adjusted")} placement="topLeft">
              <div className={styles["size-24"]}>
                <WarningCircleIcon />
              </div>
            </Tooltip>
          ) : null}
        </div>
      );
    },
    [model?.contractInfo?.contractRequestType, translate]
  );

  const items = useMemo(() => {
    const list = [
      {
        key: CollapseKey.AnnexInformation,
        label: makeCollapseTitle(CollapseKey.AnnexInformation),
        children: <AnnexInformation />,
      },
      {
        key: CollapseKey.Grounds,
        label: makeCollapseTitle(CollapseKey.Grounds),
        children: <Terms data={model?.relatedSlipInfos} />,
      },
      {
        key: CollapseKey.ContractInformation,
        label: makeCollapseTitle(
          CollapseKey.ContractInformation,
          model?.isAdjustedContractInfo
        ),
        children: <ContractInformation />,
      },
      {
        key: CollapseKey.Buyer,
        label: makeCollapseTitle(CollapseKey.Buyer, model?.isAdjustedLegalInfo),
        children: <BuyerInformation />,
      },
      {
        key: CollapseKey.Seller,
        label: makeCollapseTitle(
          CollapseKey.Seller,
          model?.isAdjustedSupplierInfo
        ),
        children: <SellerInformation />,
      },
      {
        key: CollapseKey.GoodsServicesChange,
        label: makeCollapseTitle(
          CollapseKey.GoodsServicesChange,
          model?.isAdjustedGoodsItem
        ),
        children: <GoodsService model={model} dispatch={dispatch} isEdit />,
      },
      {
        key: CollapseKey.Delivery,
        label: makeCollapseTitle(
          CollapseKey.Delivery,
          model?.isAdjustedReceiveInfo
        ),
        children: <DeliveryInformation />,
      },
      {
        key: CollapseKey.Attachment,
        label: makeCollapseTitle(
          CollapseKey.Attachment,
          model?.isAdjustedAttachment
        ),
        children: (
          <Attachments
            parentClassName={styles["bg-neutral-1"]}
            uploadIcon={UploadCloudIcon}
            attachments={model?.attachments}
            handleUpdate={(listFile) =>
              handleChangeAllField({
                attachments: listFile,
                isAdjustedAttachment: listFile?.length,
              })
            }
          />
        ),
      },
    ];

    // If goods services change is empty, remove delivery information
    if (isEmpty(model?.contractAppendixGoodsItems)) {
      list.splice(numberConstants.SIX, numberConstants.ONE);
    }

    return list;
  }, [dispatch, handleChangeAllField, makeCollapseTitle, model]);

  return (
    <div className={styles["content-container"]}>
      {/* Annex Information */}
      <AdvancedCollapseView
        items={items}
        showAll
        defaultActiveKey={items?.map((item) => item?.key)}
      />

      {isNil(model?.id) ? null : (
        <div className="px-3 pb-3">
          <Comments
            isNewLayoutVersion
            topicId={model?.id}
            topicType={TopicType.ContractAnnex}
          />
        </div>
      )}
    </div>
  );
};
