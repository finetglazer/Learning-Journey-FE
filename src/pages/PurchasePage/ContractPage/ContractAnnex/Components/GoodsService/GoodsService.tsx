...
import SelectAdjustableGoodsServicesModal from "pages/PurchasePage/ContractPage/ContractAnnex/Components/Modal/SelectAdjustableGoodsServices/SelectAdjustableGoodsServices";
...
import { AddButton } from "../../ContractAnnexDetail/Tabs/GeneralInformation/Components/AddButton";
...
import { GoodsServiceDetailDrawer } from "pages/PurchasePage/ContractPage/ContractAnnex/Components/GoodsService/GoodsServiceDetail/GoodsServiceDetailDrawer";
import { VND_CURRENCY_UNIT } from "core/config/consts";
import { formatNumberToCurrency } from "../../../../ProposalPage/ProposalCreate/ProposalGoodServices/helper";
...
export default function GoodsService<T>({
      ...
      render: (value: string, record) => {
        ...
        return (
          <LayoutCell>
            ...
            >
              <div
                onClick={() => !isTotal && handleOpenGoodsServiceModal(record)}
              >
                <OneLineText
                  className={classNames({
        ...
        return (
          <LayoutCell position="right">
            <OneLineText
              ...
              value={formatNumber(toFixedByCurrency(value, currency))}
            />
          </LayoutCell>
        );
      },
  ...
  const handleCloseGoodServiceModal = () => {
    setSelectedRowKeys([]);
    setIsContract(null);
  };

  const handleHiddenCheckbox = (record: SelectAdjustableGoodsServicesModel) => {
    if (isUndefined(record?.id)) return true;

    if (isTotalRow(record)) {
      const children = record?.children as SelectAdjustableGoodsServicesModel[];
      const listPurchases = children?.filter(
        (goodServices) =>
          isUndefined(goodServices?.purchaseItemId) ||
          isNull(goodServices?.purchaseItemId)
      );
  ...
  const rowSelections: TableRowSelection<SelectAdjustableGoodsServicesModel> = {
    ...
    onSelect: (
      ...
      const children: SelectAdjustableGoodsServicesModel[] = record?.children;
    ...
    <>
      ...
      {isBoolean(isContract) && (
        <SelectAdjustableGoodsServicesModal
          ...
          onClose={handleCloseGoodServiceModal}
