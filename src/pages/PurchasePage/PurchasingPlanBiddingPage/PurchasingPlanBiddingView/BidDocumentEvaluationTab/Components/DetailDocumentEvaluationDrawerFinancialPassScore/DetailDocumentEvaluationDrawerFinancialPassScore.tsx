...
import "./DetailDocumentEvaluationDrawerFinancialPassScore.scss";
...
import EvaluationResultsFinancialTable from "../EvaluationResultsFinancialTable/EvaluationResultsFinancialTable";
...
const DetailDocumentEvaluationDrawerFinancialPassScore = ({
  onClose,
  data,
  isEdit = false,
  roundData,
          ...
          evaluationItemResult: [
            ...
            ...(item?.evaluationMethod === EvaluationMethod.PassFail
              ? [
                  {
                    isTotalRow: true,
                    evaluationCriteriaId: "totalPointId-quotation",
                    name: "Tổng điểm báo giá",
                    passFlag: model?.summaryQuotationPoint,
                    isNumberScore: true,
                    note: model?.summaryQuotationNote,
                  },
                  {
                    isTotalRow: true,
                    evaluationCriteriaId: "totalPointId-criteria",
                    name: "Tổng điểm tiêu chí",
                    passFlag: model?.summaryFinancialPassFlag,
                    note: model?.summaryFinancialNote,
                  },
                ]
              : [
                  {
                    ...
                    evaluationCriteriaId: "totalPointId-quotation",
                    name: "Tổng điểm báo giá",
                    point: model?.summaryQuotationPoint,
                    note: model?.summaryQuotationNote,
                  },
                  {
                    isTotalRow: true,
                    evaluationCriteriaId: "totalPointId-criteria",
                    name: "Tổng điểm tiêu chí",
                    point: model?.summaryFinancialPoint,
                    note: model?.summaryFinancialNote,
                  },
                  {
                    isTotalRow: true,
                    ...
                    name: "Tổng điểm",
                    point: model?.summaryPoint,
                    note: model?.summaryNote,
                  },
                ]),
          ],
        })
  ...
  const handleSave = () => {
    ...
    const evaluationGroupResultTotalQuotation =
      evaluationGroupResultItem?.evaluationItemResult?.find(
        (item) => item?.evaluationCriteriaId === "totalPointId-quotation"
      );

    const evaluationGroupResultTotalCriteria =
      evaluationGroupResultItem?.evaluationItemResult?.find(
        (item) => item?.evaluationCriteriaId === "totalPointId-criteria"
      );

    const evaluationGroupResultTotalPoint =
      evaluationGroupResultItem?.evaluationItemResult?.find(
    ...
    if (isFinancialViewScore) {
      // Tổng điểm báo giá
      currentEvaluationResult.summaryQuotationPoint =
        evaluationGroupResultTotalQuotation?.point;

      currentEvaluationResult.summaryQuotationNote =
        evaluationGroupResultTotalQuotation?.note;

      // Tổng điểm tiêu chí
      currentEvaluationResult.summaryFinancialPoint =
        evaluationGroupResultTotalCriteria?.point;

      currentEvaluationResult.summaryFinancialNote =
        evaluationGroupResultTotalCriteria?.note;

      // Tổng điểm
      currentEvaluationResult.summaryPoint =
        evaluationGroupResultTotalPoint?.point;

      currentEvaluationResult.summaryNote =
        evaluationGroupResultTotalPoint?.note;
    }

    if (isFinancialViewPass) {
      // Tổng điểm báo giá
      currentEvaluationResult.summaryQuotationPoint =
        evaluationGroupResultTotalQuotation?.passFlag;

      currentEvaluationResult.summaryQuotationNote =
        evaluationGroupResultTotalQuotation?.note;

      // Tổng điểm tiêu chí
      currentEvaluationResult.summaryFinancialPassFlag =
        evaluationGroupResultTotalCriteria?.passFlag;
  ...
  const validateModel = () => {
    ...
    evaluationGroupResultItem?.evaluationItemResult?.forEach((itemChild) => {
      if (
        !itemChild?.isDefaultCriteria &&
        evaluationGroupResultItem?.evaluationMethod == EvaluationMethod.PassFail
      ) {
  ...
  const collapseItems = [
    ...
    },
    {
      ...
      children: (
        <EvaluationResultsFinancialTable
...
export default DetailDocumentEvaluationDrawerFinancialPassScore;
