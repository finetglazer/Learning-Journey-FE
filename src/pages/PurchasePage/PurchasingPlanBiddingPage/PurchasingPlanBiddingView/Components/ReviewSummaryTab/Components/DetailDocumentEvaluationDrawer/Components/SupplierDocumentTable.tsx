...
import "./SupplierDocumentTable.scss";
...
type Props = {
  data: FinancialProfile[] | TechnicalCompetencyProfile[];
  type?: CriteriaType;
  handleViewQuote?: () => void;
};

const SupplierDocumentTable = ({ data, type, handleViewQuote }: Props) => {
  ...
  const columns: ColumnProps<FinancialProfile>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        sorter: false,
        width: type === CriteriaType.Finance ? 145 : 178,
        render(item, record) {
          return (
            <LayoutCell>
              <div
              ...
              >
                <OneLineText
                  className={record?.quotationId && "text-blue cursor_link"}
                  value={
                    record?.quotationId
                ...
                return (
                  <LayoutCell>
                    <OneLineText
                      value={item}
                      useTooltip
                      className={classNames("text-blue", "fw-bold")}
                    />
                  </LayoutCell>
                );
              },
  ...
  return (
    <div>
      <StandardTable
        ...
        className="supplier-document-table"
      />
    </div>
  );
};
