...
import { AddIcon, emptyCloudIcon, TrashIcon } from "assets/icons";
...
const SupplierInformationTableBidding = ({
  ...
  const renderDeleteCell = useCallback(
      ...
      return (
        <LayoutCell>
          <button
            ...
            onClick={() => handleDeleteNotConfirm(record.id)}
          >
    ...
    [handleDeleteNotConfirm, isDetail]
  );
    ...
    if (isNegotiationRound) {
      return [
        ...
        },
        {
          title: (
            <UnitTitle
              title={translate("PL.txt_quotation_score")}
              isShowUnit={false}
            />
          ),
          key: ColumnKey.QUOTATION_POINT,
          dataIndex: ColumnKey.QUOTATION_POINT,
          ellipsis: true,
          width: 120,
