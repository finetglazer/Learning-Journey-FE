...
import { ReceivingGoodsDetailContextContextType } from "pages/PurchasePage/ReceivingGoods/ReceivingGoodsView/ReceivingGoodsViewContext";
...
type PropsFileTable = {
  contextValue: ReceivingGoodsDetailContextContextType;
};

export const FileTable = (props: PropsFileTable) => {
  ...
  const { model: modelMaster, dispatch: dispatchModelMaster } = contextValue;
  ...
  const handleRemoveFile = useCallback(
    ...
    [dispatch, dispatchModelMaster, model, modelMaster]
  );

  const handleUpdateModelMaster = useCallback(
    (value: any, field: string, index: number) => {
      let newValue = value;
      const payloadValue = [...modelMaster.documentGroups].map((item, idx) => {
        if (idx === index) {
          if (field === "attachments") {
            newValue = model?.documentGroups?.[index]?.attachments;
          }
      ...
      dispatchModelMaster({
        ...
        payload: {
          ...modelMaster,
          documentGroups: payloadValue,
          errors: {
            ...modelMaster.errors,
            [`documentGroups[${index}].${field}`]: null,
          },
        },
      });
    },
    [dispatchModelMaster, model?.documentGroups, modelMaster]
  );
              ...
              >
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    handleUpdateModelMaster(event, TableKey.FILES, index);
                  }}
                />
              </div>
            </LayoutCell>
