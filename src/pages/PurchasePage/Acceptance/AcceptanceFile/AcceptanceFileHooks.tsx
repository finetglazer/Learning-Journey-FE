...
export interface AcceptanceFileContextType {
  model: AcceptanceFileModel;
  dispatch: Dispatch<GeneralAction<AcceptanceFileModel>>;
...
export const AcceptanceFileContext = createContext<AcceptanceFileContextType>({
  model: null,
  dispatch: null,
});
...
export const useAcceptanceFile = (
  topicType: TopicType,
  ...
  const { model, dispatch } = detailService.useModel<AcceptanceFileModel>(
    AcceptanceFileModel,
    {
      ...new AcceptanceFileModel(),
      ...data,
    }
  );

  const { model: modelAcceptance } = useAcceptanceViewContext();
  const { model: modelReceived } = useReceivingGoodsDetailContext();
  ...
  const addNewRow = () => {
    ...
    if (
      isEqual(modelReceived?.status, numberConstants.TWO) ||
      isEqual(modelAcceptance?.status, numberConstants.THREE)
    ) {
      setModalType("ADD");
      return;
    }
  ...
  const handleShowToast = (files: File[]) => {
  ...
  const handleFileChange = useCallback(
    ...
    [updateModel, validationFileSize]
  );
  ...
  const handleAddMore = useCallback(
    (
      ...
      acceptanceRepository
        .documentGroupAddMore(model?.id, topicType, documentGroup)
        .subscribe({
          next: ({ data }) => {
            if (data) {
              notifyToast();
    ...
    [dispatch, model, notifyToChange, notifyToast, topicType]
  );
