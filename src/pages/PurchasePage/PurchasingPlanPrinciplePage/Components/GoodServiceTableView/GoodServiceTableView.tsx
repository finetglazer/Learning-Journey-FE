...
import {
  calculateTotal,
  groupDataByCategory,
} from "../../PurchasingPlanPrincipleDetail/SelectSupplierTab/Components/SelectSupplierSection/Components/GoodsServiceTable/helpers";

import { VIETNAMESE_CURRENCY } from "../../PurchasingPlanPrincipleDetail/SelectSupplierTab/Components/SelectSupplierSection/SelectSupplierSection";
...
const GoodsServiceTableView: React.FC<GoodsServiceTableViewProps> = ({
  ...
  const columns: ColumnProps<GoodPriceByCategory>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (_, record) => {
          if (record?.isTotal) {
            const totalAmount = calculateTotal(groupedDataByCategory, "amount");
            return (
              <LayoutCell position="right">
                <OneLineText
                  useTooltip
      ...
      },
      {
        ...
        render: (_, record) => {
          if (record?.isTotal) {
            const totalTax = calculateTotal(groupedDataByCategory, "totalTax");
            return (
              <LayoutCell position="right">
                <OneLineText
                  useTooltip
      ...
      },
      {
        ...
        render: (_, record) => {
          ...
          return (
            <LayoutCell position="right">
              <OneLineText useTooltip value={formatNumber(record.otherCost)} />
            </LayoutCell>
          );
        },
      },
    ...
    [translate, goodsPrices, currencyCode, groupedDataByCategory]
  );
  ...
  const dataSource = useMemo(() => {
    const totalRow: GoodPriceByCategory = {
      ...
      totalOtherCost: calculateTotal(groupedDataByCategory, "totalOtherCost"),
      category: null,
      children: [],
    };
