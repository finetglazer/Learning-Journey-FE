...
import { RequestAttachment, SettlementFile } from "./SettlementFile";
...
}: helperColumnsProps): ColumnProps<SettlementFile>[] => [
  {
    title: () => (
...
export const handleChangeItemTable = (
  ...
  const contractFiles = model?.contractFiles.map((item: SettlementFile) => {
    if (id === item?.id) {
      return {
        ...item,
        [fieldName]: value,
...
export const ViewFilesColumns = ({
  translate,
  handleDownloadFileAttached,
}: ViewFilesColumnsProps): ColumnProps<SettlementFile>[] => [
  {
