...
const AllocationAcreage = ({ selectedRowKeys, setSelectedRowKeys }: Props) => {
  ...
  useEffect(() => {
    if (model.projectName) {
      handleChangeAllField({
        ...model,
        costLineId: model.projectName?.costLines?.[0],
      });
    } else {
      handleChangeAllField({
        ...model,
  ...
  const getListCostLines = (filter: TypeFilterModel) => {
    return of(model.projectName?.costLines).pipe(
  ...
  return (
    <div>
      ...
      <div className="payment-custom_grid_9 payment-gap-12 m-t--sm">
        ...
        <div className="payment-custom_grid_9-col_3">
          <FormItem
            validateObject={utilService.getValidateObj(model, "expenseDetail")}
          >
            <InputText
        ...
        <div className="payment-custom_grid_9-col_3">
          <Select
            isSmall={false}
            label={translate("PM.payment_project_budget_item_label")}
            placeHolder={translate(
              "PM.payment_project_budget_item_placeholder"
            )}
            searchProperty="name"
            searchType=""
            valueFilter={{
        ...
        <div className="payment-custom_grid_9-col_3">
          <Select
            ...
            isSmall={false}
            label={translate("PM.payment_cost_line_label")}
            placeHolder={translate("PM.payment_cost_line_placeholder")}
            searchProperty="name"
            searchType=""
            valueFilter={{
              name: "",
            ...
            isSearch
            onChange={handleChangeSelectField({
              fieldName: "costLineId",
            })}
            getList={(search) => getListCostLines(search)}
            render={(t) => (t ? t?.code : "")}
            isEnumerable={false}
            value={model.costLineId}
            appendToBody
          />
        </div>
      </div>
