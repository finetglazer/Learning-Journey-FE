  ...
  const handleSubmitModalCostAllocation = async () => {
      ...
      switch (model.costDriver?.code) {
        ...
        case COST_DRIVER_TYPE.COST_DRIVER_QUANTITY:
          if (model.taxId != null || totalPreTaxAmount) {
            const valuePreTax = Number(totalPreTaxAmount(item));
            if (isVNDOrJPY) {
              return Number(
              ...
              return Number(
                Math.round(valuePreTax * model?.taxId?.rate * 0.01 * 100) / 100
              );
            }
          }
          return 0;
    ...
    switch (model.costDriver?.code) {
      case COST_DRIVER_TYPE.COST_DRIVER_QUANTITY:
        ...
        if (
          model.expenseDetail &&
          !regexExpenseDetail.test(model.expenseDetail)
        ) {
