...
export interface CostItemsContextType {
  modelFilter: CostItemsFilter;
  countFilter: number;
  list: CostItems[];
  count: number;
  loadingList: boolean;
  ...
  handleLoadList: () => void;
  dispatchFilter: React.Dispatch<FilterAction<CostItemsFilter>>;
...
export const CostItemsContext = React.createContext<CostItemsContextType>({
  modelFilter: new CostItemsFilter(),
  countFilter: 0,
  list: [],
  count: 0,
  loadingList: false,
...
export function useCostItemsHook() {
  ...
  const { list, count, error, loadingList, handleResetList, handleLoadList } =
    listService.useList<CostItems, CostItemsFilter>(
      reportRepository.costItemsList,
      baseFilter,
      dispatchFilter,
      getModelFilter
    );
  ...
  const handleExportExcelFile = React.useCallback(
    (filter: CostItemFilter, fileName: string) => {
      setLoadingFile(true);
      reportRepository.getCostItemsFile(filter).subscribe({
