...
const AttachedFile = () => {
  ...
  const handleUpdateList = (listFile: FileModel[]) => {
    const newListFiles = [...(model?.requestAttachments || []), ...listFile];
    handleChangeSingleField({
  ...
  if (model?.isDetail && !model?.requestAttachments?.length) return null;

  return (
    <div className="budget-plan-adjust">
      <div className="budget-plan__header">
        <span className="form-title">{translate("BG.attach_files")}</span>
      </div>
      <div className="budget-plan__attached">
        {!model?.isDetail && (
          <UploadFileCustom
            ...
            className={classNames("budget-upload-file")}
        ...
        {(model?.requestAttachments?.length > 0 || !!fileLoading.length) && (
