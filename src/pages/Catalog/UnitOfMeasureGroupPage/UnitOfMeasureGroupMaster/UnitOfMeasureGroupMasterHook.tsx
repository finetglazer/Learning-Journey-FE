...
export interface UnitOfMeasureGroupMasterContextModel {
  // for context content master
  modelFilter: UnitOfMeasureGroupFilter;
  list: UnitOfMeasureGroup[];
  count: number;
  loadingList: boolean;
  dispatchFilter: React.Dispatch<FilterAction<UnitOfMeasureGroupFilter>>;
  countFilter: number;

  rowSelection: TableRowSelection<UnitOfMeasureGroup>;
  selectedRowKeys: KeyType[];
  setSelectedRowKeys: Dispatch<SetStateAction<KeyType[]>>;

  handleLoadList: (filterParam?: UnitOfMeasureGroupFilter) => void;
  handleResetList: () => void;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  notifyToast: any;
  //for context content in detail modal
  detailModel: UnitOfMeasureGroup;
  dispatchDetailModel: React.Dispatch<GeneralAction<UnitOfMeasureGroup>>;
  isOpenModal: boolean;
  ...
  handleOpenModalDelete: (model?: UnitOfMeasureGroup) => void;
...
export const UnitOfMeasureGroupMasterContext =
  createContext<UnitOfMeasureGroupMasterContextModel>({
    // for context content master
    modelFilter: new UnitOfMeasureGroupFilter(),
    list: [],
    count: 0,
    loadingList: false,
    dispatchFilter: null,
    ...
    notifyToast: null,
    //for context content in detail modal
    detailModel: new UnitOfMeasureGroup(),
...
export function useUnitOfMeasureGroupMasterHooks() {
  const [translate] = useTranslation();

  const { notifyToast } = appMessageService.useCRUDMessage();
  const { validAction } = authorizationService.useAuthorizedAction(
    MENU_CODE.CATALOG_UNIT_OF_MEASURE_GROUP
  );
  ...
  const baseFilter: UnitOfMeasureGroupFilter = React.useMemo(() => {
    return {
      ...new UnitOfMeasureGroupFilter(),
      pageIndex: 1,
      pageSize: 10,
  ...
  const {
    ...
    handleResetList,
    handleLoadList,
  } = listService.useList<UnitOfMeasureGroup, UnitOfMeasureGroupFilter>(
    unitOfMeasureGroupRepository.getAll,
    baseFilter,
    dispatchFilter,
    getModelFilter
  );
  ...
  const { model: detailModel, dispatch: dispatchDetailModel } =
    detailService.useModel<UnitOfMeasureGroup>(UnitOfMeasureGroup);
    ...
    (id?: string, type?: "preview" | "detail") => {
      if (id) {
        unitOfMeasureGroupRepository
          .detail(id)
          .subscribe((res: UnitOfMeasureGroup) => {
            dispatchDetailModel({
  ...
  const handleCloseModal = useCallback(
    (type: "preview" | "detail") => {
      if (isOpenModal && type === "detail") {
        ...
        dispatchDetailModel({
          type: GeneralActionEnum.SET,
          payload: { ...new UnitOfMeasureGroup() },
        });
      }
  ...
  const { rowSelection, selectedRowKeys, setSelectedRowKeys } =
    listService.useRowSelection<UnitOfMeasureGroup>("checkbox", [], true);
