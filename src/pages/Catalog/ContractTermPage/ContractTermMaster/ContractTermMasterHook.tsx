...
import { ContractTermFilter } from "models/ContractTerm/ContractTermFilter";
import React, {
  createContext,
  Dispatch,
  SetStateAction,
...
export interface ContractTermMasterContextModel {
  // for context content master
  modelFilter: ContractTermFilter;
  list: ContractTerm[];
  count: number;
  loadingList: boolean;
  dispatchFilter: React.Dispatch<FilterAction<ContractTermFilter>>;
  countFilter: number;
  ...
  handleLoadList: (filterParam?: ContractTermFilter) => void;
  handleResetList: () => void;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  notifyToast: any;
  //for context content in detail modal
  detailModel: ContractTerm;
  dispatchDetailModel: React.Dispatch<GeneralAction<ContractTerm>>;
  isOpenModal: boolean;
  ...
  handleOpenModalDelete: (model?: ContractTerm) => void;
...
export const ContractTermMasterContext =
  createContext<ContractTermMasterContextModel>({
    // for context content master
    modelFilter: new ContractTermFilter(),
    list: [],
    count: 0,
    loadingList: false,
    dispatchFilter: null,
...
export function useContractTermMasterHooks() {
  ...
  const baseFilter: ContractTermFilter = React.useMemo(() => {
    return {
      ...new ContractTermFilter(),
      pageIndex: 1,
      pageSize: 10,
  ...
  const {
    ...
    handleResetList,
    handleLoadList,
  } = listService.useList<ContractTerm, ContractTermFilter>(
    contractTermRepository.getAll,
    baseFilter,
    dispatchFilter,
    getModelFilter
  );
  ...
  const { model: detailModel, dispatch: dispatchDetailModel } =
    detailService.useModel<ContractTerm>(ContractTerm);
  ...
  const handleOpenModal = useCallback(
    (id?: string, type?: "preview" | "detail") => {
      if (id) {
        contractTermRepository.detail(id).subscribe((res: ContractTerm) => {
          dispatchDetailModel({
  ...
  const { rowSelection, selectedRowKeys, setSelectedRowKeys } =
    listService.useRowSelection<ContractTerm>("checkbox", [], true);
