...
import { PromotionFilter } from "models/Promotion/PromotionFilter";
import React, {
  createContext,
  Dispatch,
  SetStateAction,
...
export interface PromotionMasterContextModel {
  ...
  rowSelection: TableRowSelection<Promotion>;
  selectedRowKeys: KeyType[];
  setSelectedRowKeys: Dispatch<SetStateAction<KeyType[]>>;

  handleLoadList: (filterParam?: PromotionFilter) => void;
  ...
  dispatchDetailModel: React.Dispatch<GeneralAction<Promotion>>;
  isOpenModal: boolean;
  ...
  handleOpenModalDelete: (model?: Promotion) => void;
...
export const PromotionMasterContext =
  createContext<PromotionMasterContextModel>({
    // for context content master
...
export function usePromotionMasterHooks() {
  ...
  const baseFilter: PromotionFilter = React.useMemo(() => {
    return {
      ...new PromotionFilter(),
      pageIndex: 1,
      pageSize: 10,
  ...
  const {
    ...
    handleResetList,
    handleLoadList,
  } = listService.useList<Promotion, PromotionFilter>(
  ...
  const { model: detailModel, dispatch: dispatchDetailModel } =
    detailService.useModel<Promotion>(Promotion);
  ...
  const handleOpenModal = useCallback(
    (id?: string, type?: "preview" | "detail") => {
      if (id) {
        promotionRepository.detail(id).subscribe((res: Promotion) => {
          dispatchDetailModel({
  ...
  const { rowSelection, selectedRowKeys, setSelectedRowKeys } =
    listService.useRowSelection<Promotion>("checkbox", [], true);
