...
import "./RankMaster.scss";

import { RankMasterContext, RankMasterContextModel } from "./RankMasterHook";
import { RankEmptySearchData } from "./RankEmptySearchData";
...
export const RankMasterTable = () => {
  const rankMaster = useContext<RankMasterContextModel>(RankMasterContext);
  ...
  const columns: ColumnProps<Rank>[] = useMemo(
    () => [
      {
        title: translate("ranks.code"),
        key: "code",
        dataIndex: "code",
        sorter: true,
        render(...params: [string, Rank, number]) {
          return (
            <LayoutCell>
              <div
                className="w-100"
      ...
      {
        title: translate("ranks.effectiveDate"),
        key: "effectiveDate",
        dataIndex: "effectiveDate",
        sorter: true,
        render(...params: [Dayjs, Rank, number]) {
          return (
            <LayoutCell>
              <OneLineText
                value={params[0] ? formatDate(params[0]) : null}
                useTooltip
              />
            </LayoutCell>
          );
