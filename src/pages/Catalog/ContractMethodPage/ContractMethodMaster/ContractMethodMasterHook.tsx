...
import { ContractMethodFilter } from "models/ContractMethod/ContractMethodFilter";
import React, {
  createContext,
  Dispatch,
  SetStateAction,
...
export interface ContractMethodMasterContextModel {
  // for context content master
  modelFilter: ContractMethodFilter;
  list: ContractMethod[];
  count: number;
  loadingList: boolean;
  dispatchFilter: React.Dispatch<FilterAction<ContractMethodFilter>>;
  countFilter: number;

  rowSelection: TableRowSelection<ContractMethod>;
  selectedRowKeys: KeyType[];
  setSelectedRowKeys: Dispatch<SetStateAction<KeyType[] | Key[]>>;

  handleLoadList: (filterParam?: ContractMethodFilter) => void;
  handleResetList: () => void;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  notifyToast: any;
  //for context content in detail modal
  detailModel: ContractMethod;
  dispatchDetailModel: React.Dispatch<GeneralAction<ContractMethod>>;
  isOpenModal: boolean;
  ...
  handleOpenModalDelete: (model?: ContractMethod) => void;
...
export const ContractMethodMasterContext =
  createContext<ContractMethodMasterContextModel>({
    // for context content master
    modelFilter: new ContractMethodFilter(),
    list: [],
    count: 0,
    loadingList: false,
    dispatchFilter: null,
    ...
    notifyToast: null,
    //for context content in detail modal
    detailModel: new ContractMethod(),
...
export function useContractMethodMasterHooks() {
  ...
  const baseFilter: ContractMethodFilter = React.useMemo(() => {
    return {
      ...new ContractMethodFilter(),
      pageIndex: 1,
      pageSize: 10,
  ...
  const {
    ...
    handleResetList,
    handleLoadList,
  } = listService.useList<ContractMethod, ContractMethodFilter>(
    contractMethodRepository.getAll,
    baseFilter,
    dispatchFilter,
    getModelFilter
  );
  ...
  const { model: detailModel, dispatch: dispatchDetailModel } =
    detailService.useModel<ContractMethod>(ContractMethod);
  ...
  const handleOpenModal = useCallback(
    (id?: string, type?: "preview" | "detail") => {
      if (id) {
        contractMethodRepository.detail(id).subscribe((res: ContractMethod) => {
          dispatchDetailModel({
  ...
  const { rowSelection, selectedRowKeys, setSelectedRowKeys } =
    listService.useRowSelection<ContractMethod>("checkbox", [], true);
