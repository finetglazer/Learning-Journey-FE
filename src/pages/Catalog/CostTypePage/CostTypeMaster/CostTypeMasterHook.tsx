...
import { CostTypeFilter } from "models/CostType/CostTypeFilter";
import React, {
  createContext,
  Dispatch,
  SetStateAction,
...
import { costTypeRepository } from "../CostTypeRepository";
...
export interface CostTypeMasterContextModel {
  // for context content master
  modelFilter: CostTypeFilter;
  list: CostType[];
  count: number;
  loadingList: boolean;
  dispatchFilter: React.Dispatch<FilterAction<CostTypeFilter>>;
  countFilter: number;

  rowSelection: TableRowSelection<CostType>;
  selectedRowKeys: KeyType[];
  setSelectedRowKeys: Dispatch<SetStateAction<KeyType[] | Key[]>>;

  handleLoadList: (filterParam?: CostTypeFilter) => void;
  handleResetList: () => void;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  notifyToast: any;
  //for context content in detail modal
  detailModel: CostType;
  dispatchDetailModel: React.Dispatch<GeneralAction<CostType>>;
  isOpenModal: boolean;
  ...
  handleOpenModalDelete: (model?: CostType) => void;
...
export const CostTypeMasterContext = createContext<CostTypeMasterContextModel>({
  // for context content master
  modelFilter: new CostTypeFilter(),
  list: [],
  count: 0,
  loadingList: false,
  dispatchFilter: null,
...
export function useCostTypeMasterHooks() {
  const [translate] = useTranslation();

  const { notifyToast } = appMessageService.useCRUDMessage();
  const { validAction } =
    authorizationService.useAuthorizedAction("CATALOG_COST_TYPE");
  ...
  const baseFilter: CostTypeFilter = React.useMemo(() => {
    return {
      ...new CostTypeFilter(),
      pageIndex: 1,
      pageSize: 10,
  ...
  const {
    ...
    handleResetList,
    handleLoadList,
  } = listService.useList<CostType, CostTypeFilter>(
    costTypeRepository.getAll,
    baseFilter,
    dispatchFilter,
    getModelFilter
  );
  ...
  const { model: detailModel, dispatch: dispatchDetailModel } =
    detailService.useModel<CostType>(CostType);
  ...
  const handleOpenModal = useCallback(
    (id?: string, type?: "preview" | "detail") => {
      if (id) {
        costTypeRepository.detail(id).subscribe((res: CostType) => {
          dispatchDetailModel({
  ...
  const { rowSelection, selectedRowKeys, setSelectedRowKeys } =
    listService.useRowSelection<CostType>("checkbox", [], true);
