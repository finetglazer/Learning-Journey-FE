...
const BankDetail = () => {
  ...
  const { loading, handleChangeSingleField, handleSave } = useBankDetailHook(
    model,
    dispatchModel,
    handleCloseModal,
    handleLoadList
  ...
  return (
    <>
      <Modal
        ...
        title={
          model?.id
            ? `${translate("banks.update")}`
            : `${translate("banks.create")}`
        }
        size={800}
        centered
      ...
      >
        <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }} className="p-x--sm">
          <Col lg={24} className="m-b--sm d-flex">
            ...
            />
            <span className="m-l--xs">{translate("banks.active")}</span>
          </Col>
          ...
          <Col lg={12} className="m-b--sm m-t--xs">
            <Checkbox
              label={translate("banks.isInternal")}
