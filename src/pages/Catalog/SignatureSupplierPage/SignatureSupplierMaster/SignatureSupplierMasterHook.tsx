...
import { SignatureSupplierFilter } from "models/SignatureSupplier/SignatureSupplierFilter";
import React, {
  createContext,
  Dispatch,
  SetStateAction,
...
import { signatureSupplierRepository } from "../SignatureSupplierRepository";
...
export interface SignatureSupplierMasterContextModel {
  // for context content master
  modelFilter: SignatureSupplierFilter;
  list: SignatureSupplier[];
  count: number;
  loadingList: boolean;
  dispatchFilter: React.Dispatch<FilterAction<SignatureSupplierFilter>>;
  countFilter: number;

  rowSelection: TableRowSelection<SignatureSupplier>;
  selectedRowKeys: KeyType[];
  setSelectedRowKeys: Dispatch<SetStateAction<KeyType[] | Key[]>>;

  handleLoadList: (filterParam?: SignatureSupplierFilter) => void;
  handleResetList: () => void;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  notifyToast: any;
  //for context content in detail modal
  detailModel: SignatureSupplier;
  dispatchDetailModel: React.Dispatch<GeneralAction<SignatureSupplier>>;
  isOpenModal: boolean;
  ...
  handleOpenModalDelete: (model?: SignatureSupplier) => void;
...
export const SignatureSupplierMasterContext =
  createContext<SignatureSupplierMasterContextModel>({
    // for context content master
    modelFilter: new SignatureSupplierFilter(),
    list: [],
    count: 0,
    loadingList: false,
    dispatchFilter: null,
    ...
    notifyToast: null,
    //for context content in detail modal
    detailModel: new SignatureSupplier(),
...
export function useSignatureSupplierMasterHooks() {
  const [translate] = useTranslation();

  const { notifyToast } = appMessageService.useCRUDMessage();
  const { validAction } = authorizationService.useAuthorizedAction(
    MENU_CODE.CATALOG_SIGNATURE_SUPPLIER
  );
  // Filter
  ...
  const baseFilter: SignatureSupplierFilter = React.useMemo(() => {
    return {
      ...new SignatureSupplierFilter(),
      pageIndex: 1,
      pageSize: 10,
  ...
  const {
    ...
    handleResetList,
    handleLoadList,
  } = listService.useList<SignatureSupplier, SignatureSupplierFilter>(
    signatureSupplierRepository.getAll,
    baseFilter,
    dispatchFilter,
    getModelFilter
  );
  ...
  const { model: detailModel, dispatch: dispatchDetailModel } =
    detailService.useModel<SignatureSupplier>(SignatureSupplier);
  ...
  const handleOpenModal = useCallback(
    (id?: string, type?: "preview" | "detail") => {
      if (id) {
        signatureSupplierRepository
          .detail(id)
          .subscribe((res: SignatureSupplier) => {
            dispatchDetailModel({
  ...
  const { rowSelection, selectedRowKeys, setSelectedRowKeys } =
    listService.useRowSelection<SignatureSupplier>("checkbox", [], true);
