...
import { BusinessDepartmentFilter } from "models/BusinessDepartment/BusinessDepartmentFilter";
import React, {
  createContext,
  Dispatch,
  SetStateAction,
...
export interface BusinessDepartmentMasterContextModel {
  // for context content master
  modelFilter: BusinessDepartmentFilter;
  list: BusinessDepartment[];
  count: number;
  loadingList: boolean;
  dispatchFilter: React.Dispatch<FilterAction<BusinessDepartmentFilter>>;
  countFilter: number;

  rowSelection: TableRowSelection<BusinessDepartment>;
  selectedRowKeys: KeyType[];
  setSelectedRowKeys: Dispatch<SetStateAction<KeyType[] | Key[]>>;

  handleLoadList: (filterParam?: BusinessDepartmentFilter) => void;
  handleResetList: () => void;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  notifyToast: any;
  //for context content in detail modal
  detailModel: BusinessDepartment;
  dispatchDetailModel: React.Dispatch<GeneralAction<BusinessDepartment>>;
  isOpenModal: boolean;
  ...
  handleOpenModalDelete: (model?: BusinessDepartment) => void;
...
export const BusinessDepartmentMasterContext =
  createContext<BusinessDepartmentMasterContextModel>({
    // for context content master
    modelFilter: new BusinessDepartmentFilter(),
    list: [],
    count: 0,
    loadingList: false,
    dispatchFilter: null,
    ...
    notifyToast: null,
    //for context content in detail modal
    detailModel: new BusinessDepartment(),
...
export function useBusinessDepartmentMasterHooks() {
  const [translate] = useTranslation();

  const { notifyToast } = appMessageService.useCRUDMessage();

  const { validAction } = authorizationService.useAuthorizedAction(
    MENU_CODE.CATALOG_BUSINESS_DEPARTMENT
  );
  // Filter
  ...
  const baseFilter: BusinessDepartmentFilter = React.useMemo(() => {
    return {
      ...new BusinessDepartmentFilter(),
      pageIndex: 1,
      pageSize: 10,
  ...
  const {
    ...
    handleResetList,
    handleLoadList,
  } = listService.useList<BusinessDepartment, BusinessDepartmentFilter>(
    businessDepartmentRepository.getAll,
    baseFilter,
    dispatchFilter,
    getModelFilter
  );
  ...
  const { model: detailModel, dispatch: dispatchDetailModel } =
    detailService.useModel<BusinessDepartment>(BusinessDepartment);
  ...
  const handleOpenModal = useCallback(
    (id?: string, type?: "preview" | "detail") => {
      if (id) {
        businessDepartmentRepository
          .detail(id)
          .subscribe((res: BusinessDepartment) => {
            dispatchDetailModel({
              ...
              payload: {
                ...
                workflowDefinition: {
                  ...
                  name: res?.data?.workflowDefinitionName,
                },
                workflowActivity: {
                  id: res?.data?.workflowActivityId,
                  name: res?.data?.workflowActivityName,
                },
              },
            });
          });
  ...
  const handleCloseModal = useCallback(
    (type: "preview" | "detail") => {
      if (isOpenModal && type === "detail") {
        ...
        dispatchDetailModel({
          type: GeneralActionEnum.SET,
          payload: { ...new BusinessDepartment() },
        });
      }
  ...
  const { rowSelection, selectedRowKeys, setSelectedRowKeys } =
    listService.useRowSelection<BusinessDepartment>("checkbox", [], true);
