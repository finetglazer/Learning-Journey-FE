...
export const useWarrantyTypeDetailHooks = (
  ...
  const { model, dispatch } = detailService.useModel<WarrantyType>(
    WarrantyType,
    {
      ...new WarrantyType(),
      isActive: true,
  ...
  const getDetail = useCallback(() => {
    setLoading(true);
    warrantyTypeRepository.getDetail(id).subscribe({
      next: (response: WarrantyType) => {
        dispatch({
    ...
    warrantyTypeRepository
      ...
      .subscribe({
        ...
        error: (error: AxiosError) =>
          handleError<WarrantyType>({ model, error, handleChangeAllField }),
      });
  };

  const updateWarrantyType = () => {
    setLoading(true);
    warrantyTypeRepository
      .update(model)
      .pipe(finalize(() => setLoading(false)))
