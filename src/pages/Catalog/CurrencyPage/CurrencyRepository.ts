/* eslint-disable @typescript-eslint/no-explicit-any */
import ConfigStore from "core/config/ConfigStore";
import { httpConfig } from "core/config/http";
import { trimStringFieldObject } from "core/helpers/json";
import { Currency, CurrencyFilter } from "models/Currency";

import { Repository } from "react-3layer-common";
import { Observable } from "rxjs";
import nameof from "ts-nameof.macro";

export const API_CURRENCY_PREFIX = "/master/currency";

export class CurrencyRepository extends Repository {
  constructor() {
    super(httpConfig);
    this.baseURL = `${ConfigStore.getInstance().get(
      "baseApiUrl"
    )}${API_CURRENCY_PREFIX}`;
  }

  // Get all
  public getAll = (data?: CurrencyFilter): Observable<any> => {
    const requestBody = {
      ...data,
      status: data?.statusId?.length > 0 ? data?.statusId : [],
    };
    return this.http.post(
      nameof(this.getAll),
      trimStringFieldObject(requestBody)
    );
  };

  // get
  public detail = (id: string): Observable<any> => {
    const endpoint = `/${id}`;
    return this.http.get(endpoint);
  };

  // delete
  public delete = (ids: string[]): Observable<any> => {
    return this.http.delete("", { data: ids });
  };

  public createCurrency = (data: any): Observable<any> => {
    return this.http.post("", data);
  };

  public updateCurrency = (data: any): Observable<any> => {
    return this.http.put(`${data?.id}`, data);
  };

  public saveCurrency = (data: any): Observable<any> => {
    return data?.id ? this.updateCurrency(data) : this.createCurrency(data);
  };

  public position = (data: any): Observable<any> => {
    return this.http.post("/position", data);
  };

  // Get all
  public getDropdown = (data?: CurrencyFilter): Observable<any> => {
    return this.http
      .get(nameof(this.getDropdown), { data })
      .pipe(Repository.responseDataMapper<Currency[]>());
  };
}

export const currencyRepository = new CurrencyRepository();
