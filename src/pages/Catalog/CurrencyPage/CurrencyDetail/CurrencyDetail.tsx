...
import { useCurrencyDetailHook } from "./CurrencyDetailHook";
import {
  ...
  return (
    <>
      <Modal
        ...
        title={
          model?.id
            ? `${translate("currencies.update")}`
            : `${translate("currencies.create")}`
        }
        size={800}
        centered
      ...
      >
        <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }} className="p-x--sm">
          <Col lg={24} className="m-b--sm d-flex">
            ...
            />
            <span className="m-l--xs">{translate("currencies.active")}</span>
          </Col>
