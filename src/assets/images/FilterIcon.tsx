/* eslint-disable react/no-unknown-property */

import { SVGIconProps } from "assets/icons/SVGIconProps";

export const FilterIcon = (props: SVGIconProps) => {
  const { width = 16, height = 16, fillColor = "#343330" } = props;

  return (
    <svg
      width={width}
      height={height}
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M6.5 5.5H9.5"
        stroke={fillColor}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M14.3494 9.64485L11.6213 3.43735C11.3399 3.15607 10.9584 2.99805 10.5606 2.99805C10.1628 2.99805 9.7813 3.15607 9.5 3.43735V10.4999"
        stroke={fillColor}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M6.49977 10.4999V3.43735C6.21846 3.15607 5.83695 2.99805 5.43914 2.99805C5.04133 2.99805 4.65982 3.15607 4.37852 3.43735L1.65039 9.64485"
        stroke={fillColor}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M4 13C5.38071 13 6.5 11.8807 6.5 10.5C6.5 9.11929 5.38071 8 4 8C2.61929 8 1.5 9.11929 1.5 10.5C1.5 11.8807 2.61929 13 4 13Z"
        stroke={fillColor}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M12 13C13.3807 13 14.5 11.8807 14.5 10.5C14.5 9.11929 13.3807 8 12 8C10.6193 8 9.5 9.11929 9.5 10.5C9.5 11.8807 10.6193 13 12 13Z"
        stroke={fillColor}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
};
